...
class UIDrawer:
    ...
    def draw_label_and_progress(
        self, frame: np.ndarray, x: int, y: int, w: int, h: int, label: str, confidence: float
    ) -> None:
    ...
    def draw_cooldown_bar(self, frame: np.ndarray) -> None:
        ...
        cv2.putText(
            frame,
            f"Cooldown: {max(0, self.cooldown_timer.cooldown - cooldown):.1f}s",
            (10, frame.shape[0] - 5),
        ...
        if self.cooldown_timer.last_action_label:
            ...
            cv2.putText(
                frame,
                action_text,
                (10, frame.shape[0] - bar_height - 10),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.7,
                (255, 255, 0),
                2,
